...
**1. Méthodes de ré-échantillonnage**
  ...
  - [Cours 5](#c5) (26/01/2026)  
  
**2. Méthodes de Monte-Carlo**
  - [Cours 6](#c6) (27/01/2026)
  - [Cours 7](#c7) (03/02/2026)  
  - [Cours 8](#c8) (06/02/2026)  
  - [Cours 9](#c9) (09/02/2026)  
  - [Cours 10](#c10) (10/02/2026) 
  - [Cours 11](#c11) (16/02/2026)
  - [Cours 12](#c12) (02/03/2026)
  - [Cours 13](#c13) (03/03/2026)
  - [Cours 14](#c14) (09/03/2026)
]
.pull-right[
**3. Statistiques bayésiennes**
  - [Cours 15](#c15) (13/03/2026)
  - [Cours 16](#c16) (16/03/2026)
  - [Cours 17](#c17) (17/03/2026)
  - [Cours 18](#c18) (23/03/2026)
  - [Cours 19](#c19) (24/03/2026)
]
...
> Soit $X$ une v.a. de densité $f$, $g$ une densité de probabilité et $M$ t.q. $\forall x, f(x) \leq M g(x)$. Alors pour simuler selon la loi $f$ il suffit de :
1. simuler $Y \sim g$
2. simuler $U \sim \mathcal{U}([0,1])$
3. si $0 < U < f(Y)/Mg(Y)$, poser $X=Y$, sinon reprendre l'étape 1.

RQ. : cela marche aussi si on connaît $f$ à une constante multiplicative près.
...
RQ : on peut chercher $\tilde{M}$ telle que $e^{-x^2/2} \leq \tilde{M} e^{-x}$ pour $x >0$.
...
En augmentant la taille de l'échantillon ($n=100000$) :
...
u <- runif(n,0,2*pi)
mi = mean((sin(u)^2+2*cos(3*u))^2)*2*pi
vari = var((sin(u)^2+2*cos(3*u))^2)*4*pi*pi/n
```
...
Y <- rexp(n)
hY <- ifelse(Y < 1,Y^(-a),0)
print(paste0("Estimation : ",mean(hY),", variance : ",var(hY)/n))
```

En simulant selon une loi exponentielle à support dans $\mathbb{R}^+$, on a "perdu" certains points : ceux qui sont plus grands que 1. Cela correspondà une proportion :
...
Prenons $f(x) = (1-a) \ x^{-a} \mathbb{1}_{[0,1]}(x)$ et $\tilde{h}(x) = e^{-x}/ (1-a)$
...
<span style="color:#16A085">**fin du cours 9**</span>
...
<span style="color:#16A085">**fin du cours 10**</span>
...
<span style="color:#16A085">**fin du cours 11**</span>
...
dx <- data.frame(x=c(X,X[(N-200+1):N]),n=c(1:N,(N-200+1):N),type=c(rep(".Toute la trajectoire",N),rep("200 dernières itérations",200)))
...
p_traj <- ggplot(dx[1:N,], aes(x=n, y=x)) +
  geom_step() +
  labs(title="Évolution de la chaîne",
       subtitle="Temps: {frame_along}",
       y="État") +
  transition_reveal(n)
...
hist_list <- map(1:N, function(i){
  
  h <- hist(X[1:i], breaks = breaks, plot = FALSE)
...
df_hist <- bind_rows(hist_list)

x_grid <- seq(0, 1, length.out = 400)

df_true <- data.frame(
  x = x_grid,
  y = dbeta(x_grid, 2.5, 8)
)
...
p_hist <- ggplot(df_hist,
                 aes(xmin = xmin,
                     ...
                     ymin = 0,
                     ymax = density)) +
  geom_rect(fill = "grey", color = "white") +
    geom_line(data = df_true,
            aes(x = x, y = y),
            color = "black",
            inherit.aes = FALSE,
            linewidth = 1.2) +
  ylim(c(0,max(df_true$y))) +
  
  labs(title = "Histogramme cumulatif",
       subtitle = "Temps : {current_frame}",
       y = "Densité") +
  transition_manual(n)

library(magick)

a <- animate(p_traj,renderer = magick_renderer())
b <- animate(p_hist,renderer = magick_renderer())
...
---
# EP et Metropolis-Hastings

Exploration de l'espace, cible : $f(x) = 0.3 \mathcal{N}((-3,-2)^T, I_2) + 0.7\mathcal{N}((5,3)^T, I_2)$

**Echantillonnage préférentiel**

--

```{r, echo=FALSE, fig.height=6, fig.width=6, fig.retina=2, fig.align='center'}
library(mvtnorm)

f <- function(x){
  0.3*dmvnorm(x,c(-3,-2),diag(c(1,1))) + 0.7*dmvnorm(x,c(5,3),diag(c(1,1)))
}

n <- 5000
cand <- matrix(rmvnorm(n,c(0,0),diag(c(10,10))),nr=n,nc=2)

w <- f(cand)/dmvnorm(cand,c(0,0),diag(c(10,10)))#dunif(cand[,1],-10,10)/dunif(cand[,2],-10,10)

library(ggplot2)
d <- data.frame(cand)
d$w <- w/sum(w)

ggplot(data=d,aes(x=X1,y=X2)) + geom_point(alpha=0.5) + theme(legend.position = "none")
```
...
```{r, echo=FALSE, fig.height=6, fig.width=6, fig.retina=2, fig.align='center'}
ggplot(data=d,aes(x=X1,y=X2,size=w)) + geom_point(alpha=0.5) + 
  scale_size_continuous(range = c(0.25, 7)) + theme(legend.position = "none")
```
...
```{r, echo=FALSE, fig.height=3, fig.width=3, fig.retina=2, fig.align='center'}
# MH
seed <- 0
x <- matrix(c(0,0),nr=1)

for (i in 1:n){
  cand <- x[i,] + rmvnorm(1,mean=c(0,0),diag(c(3,3)))
  alpha <- f(cand)/f(x[i,])
  u <- runif(1)
  xnew <- cand*(u<alpha)+x[i,]*(u>alpha)
  x <- rbind(x,xnew)
}
...
#plot(x,type="l",xlim=c(-10,10),ylim=c(-10,10))

library(gganimate)

d2 <- data.frame(x)
d2$iteration <- 1:nrow(d2)

p <- ggplot(data=d2,aes(x=X1,y=X2)) + geom_point() + geom_path(alpha=0.2) + xlim(c(-10,10)) + ylim(c(-10,10)) + shadow_trail(alpha = 0.1) + transition_reveal(iteration) + exit_fade()
p
```
...
.pull-right[
```{r, echo=FALSE}
ggplot(data=d2,aes(x=X1,y=X2)) + geom_point() + geom_path(alpha=0.2) + xlim(c(-10,10)) + ylim(c(-10,10))
```
]


---
# AR et Metropolis-Hastings

.pull-left[
Acceptation-rejet : exploration globale 
...
dcircle <- data.frame(X1=xt,X2=yt)
dxy <- data.frame(X1=x,X2=y,acc=acc)

ggplot(data=dxy,aes(x=X1,y=X2,col=as.factor(acc))) + geom_point() + geom_point(data=dcircle,inherit.aes = FALSE, aes(x=X1,y=X2)) + theme(legend.position = "none")
```
]

.pull-right[
Metropolis-Hastings : exploration locale
```{r, echo=FALSE, message=FALSE, warning=FALSE}
### MH
n <- 1000
rej <- data.frame(X1=numeric(),X2=numeric(),iteration=numeric())
XY <- matrix(rep(0,2*n),nc=2); XY[1,] <- runif(2,-1,1)
while (XY[1,1]^2 + XY[1,2]^2 >1) XY[1,] <- runif(2,-1,1)
for (i in 2:n){
  Y <- runif(2,-0.1,0.1) + XY[i-1,]
  acc <- (Y[1]^2 + Y[2]^2 < 1)*1
  XY[i,] <- acc*Y + (1-acc)*XY[i-1,]
  if (!acc) {rej <- rbind(rej,data.frame(X1=Y[1],X2=Y[2],iteration=i))}
}
...
d_all <- left_join(d,rej)

df_plot <- d %>%
  group_by(X1, X2) %>%
  mutate(taille_point = n()) %>% # Compte le nombre de points aux mêmes coordonnées
  ungroup()
...
rej_plot <- full_join(rej_plot,data.frame(iteration=1:n))
rej_plot$taille_point[is.na(rej_plot$taille_point)] <- 0

p <- ggplot(data=df_plot,aes(x=X1,y=X2)) + geom_point(aes(group = seq_along(iteration),size=taille_point)) + 
  geom_point(data=rej_plot,col="red", aes(x=X1,y=X2,group = seq_along(iteration))) +
  geom_path(alpha=0.2) + geom_point(data=dcircle,inherit.aes = FALSE, aes(x=X1,y=X2)) + 
  scale_size_continuous(range = c(1, max(df_plot$taille_point))) + theme(legend.position = "none") + transition_reveal(iteration) + exit_fade()
p
```
]
...
<span style="color:#16A085">**fin du cours 12**</span>
...
Cas où $q$ est indépendant de $X_{n-1}$ &rarr; $q(x | y) = q(x)$
 ...
 - voir l'exemple de la slide 133 (loi cible $\text{Beta}(2.5,8)$ et loi de proposition $q$ la loi $\mathcal{U}([0,1])$)
...
Cas où $q$ est symétrique en $x$ et $y$ &rarr; $q(y | x) = q(x | y)$
 ...
 - voir l'exemple de la slide 136 (loi cible $\text{Beta}(2.5,8)$ et loi de proposition $q$ la loi $\mathcal{U}([0.8X_{n-1},1.2X_{n-1}])$)
...
**Exemple** : loi cible $f$ la loi uniforme sur le disque unité, et $q(\cdot \mid x) = \mathcal{U}([x-\delta,x+\delta]^2)$ pour différent choix de $\delta$ et une chaîne de taille $N=1000$

```{r, echo=FALSE, message=FALSE, warning=FALSE, fig.height=5, fig.width=14, fig.retina=2, fig.align='center'}
### MH
n <- 1000
set.seed(03032026)
rej1 <- rej2 <- rej3 <- data.frame(X1=numeric(),X2=numeric(),iteration=numeric())
XY1 <- matrix(rep(0,2*n),nc=2); XY1[1,] <- runif(2,-1,1)
XY2 <- matrix(rep(0,2*n),nc=2); XY2[1,] <- runif(2,-1,1)
XY3 <- matrix(rep(0,2*n),nc=2); XY3[1,] <- runif(2,-1,1)
while (XY1[1,1]^2 + XY1[1,2]^2 >1) XY1[1,] <- runif(2,-1,1)
while (XY2[1,1]^2 + XY2[1,2]^2 >1) XY2[1,] <- runif(2,-1,1)
while (XY3[1,1]^2 + XY3[1,2]^2 >1) XY3[1,] <- runif(2,-1,1)

for (i in 2:n){
  Y1 <- runif(2,-0.1,0.1) + XY1[i-1,]
  Y2 <- runif(2,-0.05,0.05) + XY2[i-1,]
  Y3 <- runif(2,-0.3,0.3) + XY3[i-1,]
  acc1 <- (Y1[1]^2 + Y1[2]^2 < 1)*1
  acc2 <- (Y2[1]^2 + Y2[2]^2 < 1)*1
  acc3 <- (Y3[1]^2 + Y3[2]^2 < 1)*1
  XY1[i,] <- acc1*Y1 + (1-acc1)*XY1[i-1,]
  XY2[i,] <- acc2*Y2 + (1-acc2)*XY2[i-1,]
  XY3[i,] <- acc3*Y3 + (1-acc3)*XY3[i-1,]
  if (!acc1) {rej1 <- rbind(rej1,data.frame(X1=Y1[1],X2=Y1[2],iteration=i))}
  if (!acc2) {rej2 <- rbind(rej2,data.frame(X1=Y2[1],X2=Y2[2],iteration=i))}
  if (!acc3) {rej3 <- rbind(rej3,data.frame(X1=Y3[1],X2=Y3[2],iteration=i))}
}

d1 <- data.frame(XY1); d1$var = "delta = 0.1"
d2 <- data.frame(XY2); d2$var = "delta = 0.05"
d3 <- data.frame(XY3); d3$var = "delta = 0.3"
d <- rbind(d1,d2,d3)
d$iteration <- rep(1:n,3)

d$acc <- TRUE

rej1$var = "delta = 0.1"; rej2$var = "delta = 0.05"; rej3$var = "delta = 0.3"
rej <- rbind(rej1,rej2,rej3)
rej$acc <- FALSE
...
rej_plot <- full_join(rej_plot,data.frame(iteration=rep(1:n,3),var=rep(c("delta = 0.1","delta = 0.05","delta = 0.3"),each=n)))
rej_plot$taille_point[is.na(rej_plot$taille_point)] <- 0

library(gifski)

p <- ggplot(data=df_plot,aes(x=X1,y=X2)) + geom_point(aes(group = seq_along(iteration),size=taille_point)) + facet_grid(~var) +
  geom_point(data=rej_plot,col="red", aes(x=X1,y=X2,group = seq_along(iteration))) +
  geom_path(alpha=0.2) + geom_point(data=dcircle,inherit.aes = FALSE, aes(x=X1,y=X2)) + 
  scale_size_continuous(range = c(1, max(df_plot$taille_point))) + theme(legend.position = "none") + transition_reveal(iteration) + exit_fade()
...
Taux d'acceptation empirique : `r 100-100*nrow(rej2)/n`% - `r 100-100*nrow(rej1)/n`% - `r 100-100*nrow(rej3)/n`%
...
```{r, echo=FALSE, fig.height=3,fig.width=4,fig.retina=1.5,fig.align='center'}
n <- 1000
X <- runif(1)
U <- 0
for (i in 2:n){
...
dsl <- data.frame(x=X,u=U,it=1:n)
ddens <- data.frame(x=seq(0,1,length.out=1000),u=dbeta(seq(0,1,length.out=1000),5,2))
p <- ggplot(data=dsl,aes(x=x,y=u)) + geom_point(aes(group = seq_along(it))) + geom_path(alpha=0.05) + geom_line(data=ddens,inherit.aes = FALSE,aes(x=x,y=u),col="red",linewidth=1.1) + shadow_trail(alpha = 0.05) + transition_reveal(it) + exit_fade()
animate(p)
```

<span style="color:#16A085">**fin du cours 13**</span>
...
- en traçant l'autocorrélation de la chaîne et en élagant (on enlève le burn-in)

```{r, fig.height=4, fig.width=8, fig.retina=2, fig.align='center'}
par(mfrow=c(1,2))
acf(X,main="Toute la chaîne")
acf(X[500:2000],main="Sans le burn-in")
```


<span style="color:#16A085">**fin du cours 14**</span>
...
<span style="color:#16A085">**fin du cours 15**</span>
...
Estimateurs ponctuels : moyenne a posteriori et maximum a posterior

```{r , echo=FALSE, fig.width=12, fig.height=6, fig.align='center', fig.retina=2, message=F,warning=F}
a<-7.75;b<-12.25;nobs=5;xobs=2
postmean <- (xobs+a)/(nobs+a+b)
map <- (xobs+a-1)/(nobs+a+b-2)
dpost <- data.frame(x=seq(0,1,0.001),y=dbeta(x,2+a,3+b),mod=1)
a2<-2;b2<-10;nobs=5;xobs=2
postmean2 <- (xobs+a2)/(nobs+a2+b2)
map2 <- (xobs+a2-1)/(nobs+a2+b2-2)
dpost2 <- data.frame(x=seq(0,1,0.001),y=dbeta(x,2+a2,3+b2),mod=2)
dpost <- rbind(dpost,dpost2)
dseg <- data.frame(y=c(dbeta(postmean,xobs+a,nobs-xobs+b),
                       dbeta(map,xobs+a,nobs-xobs+b),
                       dbeta(postmean2,xobs+a2,nobs-xobs+b2),
                       dbeta(map2,xobs+a2,nobs-xobs+b2)),
                    x=c(postmean,map,postmean2,map2),
                    mod = c(1,1,2,2),
                   type=c("Moyenne a posteriori","Maximum a posteriori","Moyenne a posteriori","Maximum a posteriori"))
ggplot(data=dpost,aes(x=x,y=y)) + geom_line(lwd=1,col="#77bc65") + theme(legend.position = "bottom") + geom_segment(data=dseg,aes(x=x,xend=x,y=0,yend=y,linetype=type),linewidth = 0.75) +
  #geom_segment(aes(linetype="Moyenne a posteriori"),y=0,yend=,x=postmean,xend=postmean, col="#77bc65") + geom_segment(aes(linetype="Maximum a posteriori"),y=0,yend=,x=map,xend=map,col="#77bc65") +
   #geom_segment(aes(linetype="Moyenne a posteriori"),y=0,yend=,x=postmean2,xend=postmean2, col="#77bc65") + geom_segment(aes(linetype="Maximum a posteriori"),y=0,yend=,x=map2,xend=map2,col="#77bc65") +
  scale_linetype_discrete("") + facet_grid(~as.factor(mod))
```
...
```{r}
qbeta(c(0.025,0.975),xobs+a,nobs-xobs+b)
```
 - en utilisant la région de plus forte densité : (librairie `HDInterval` sous R et `PyMC` sous Python)
```{r}
library(HDInterval)
hdi(qbeta,0.95,shape1=xobs+a,shape2=nobs-xobs+b)
```

---
# Modèle Beta-binomial

Représentation graphique de l'intervalle de crédibilité


```{r , echo=FALSE, fig.width=6, fig.height=5, fig.align='center', fig.retina=2, warning=FALSE, message=FALSE}
a<-7.75;b<-12.25;nobs=50;xobs=20
dpost <- data.frame(x=seq(0,1,0.001),y=dbeta(x,xobs+a,nobs-xobs+b),mod=1)
q1 <- qbeta(0.025,xobs+a,nobs-xobs+b); q2 <- qbeta(0.975,xobs+a,nobs-xobs+b)
qhdi <- hdi(qbeta,0.95,shape1=xobs+a,shape2=nobs-xobs+b)
ggplot(data=dpost,aes(x=x,y=y)) + geom_line(lwd=1,col="#77bc65") + theme(legend.position = "bottom") + stat_function(fun = dbeta,xlim = c(q1,q2), args = list(shape1=xobs+a,shape2=nobs-xobs+b), geom = "area", fill="#77bc65", alpha=0.15) +
  stat_function(fun = dbeta,xlim = qhdi, args = list(shape1=xobs+a,shape2=nobs-xobs+b), geom = "area", fill="#77bc65", alpha=0.15) + xlim(c(0.15,0.6)) + geom_segment(x=0.45,xend=0.55,y=2,yend=4,lwd=0.1,col="#77bc65",lwd=3) +
  annotate(geom="text",x=0.55,y=4.2,label="1-alpha",parse=TRUE,size=5)
```
...
La différence entre l'intervalle de crédibilité basé sur les quantiles et celui basé sur l'HDI est plus visible en cas de distribution asymétrique :

```{r , echo=FALSE, fig.width=6, fig.height=5, fig.align='center', fig.retina=2, message=F,warning=F}
a<-1;b<-3;nobs=3;xobs=1
q1 <- qbeta(0.025,xobs+a,nobs-xobs+b); q2 <- qbeta(0.975,xobs+a,nobs-xobs+b)
qhdi <- hdi(qbeta,0.95,shape1=xobs+a,shape2=nobs-xobs+b)
dpost <- data.frame(x=x,y=dbeta(x,xobs+a,nobs-xobs+b))
ggplot(data=dpost,aes(x=x,y=y)) + geom_line(lwd=1,col="#77bc65") + theme(legend.position = "bottom") + stat_function(fun = dbeta,xlim = c(q1,q2), args = list(shape1=xobs+a,shape2=nobs-xobs+b), geom = "area", fill="#77bc65", alpha=0.15) +
  stat_function(fun = dbeta,xlim = qhdi, args = list(shape1=xobs+a,shape2=nobs-xobs+b), geom = "area", fill="#77bc65", alpha=0.15) + geom_segment(aes(linetype="IC quantiles"), x=q1,xend=q1,y=0,yend=dbeta(q1,xobs+a,nobs-xobs+b),col="#77bc65",lwd=0.5) +
  geom_segment(aes(linetype="IC quantiles"), x=q2,xend=q2,y=0,yend=dbeta(q2,xobs+a,nobs-xobs+b),col="#77bc65",lwd=0.5) +
  geom_segment(aes(linetype="IC HDI"), x=qhdi[1],xend=qhdi[1],y=0,yend=dbeta(qhdi[1],xobs+a,nobs-xobs+b),col="#77bc65",lwd=0.5) +
  geom_segment(aes(linetype="IC HDI"), x=qhdi[2],xend=qhdi[2],y=0,yend=dbeta(qhdi[2],xobs+a,nobs-xobs+b),col="#77bc65",lwd=0.5) + scale_linetype_discrete("")
```

 - Intervalle de crédibilité donné par les quantiles : [0.043 ; 0.641]
 - Intervalle de crédibilité donné par HDI : [0.018 ; 0.591]
...
Test d'hypothèses : $H_0 : \theta \leq 0.5 \quad \text{vs.} \quad H_1 : \theta > 0.5$

 - risque relatif a posteriori : $\frac{\mathbb{P}(\theta \leq 0.5 | x)}{\mathbb{P}(\theta > 0.5 | x)} = \frac{\int \mathbb{1}_{t\leq 0.5} p(\theta | x)d\theta}{\int \mathbb{1}_{t > 0.5} p(\theta | x)d\theta}$
```{r}
pbeta(0.5,20+7.75,30+12.25)/(1-pbeta(0.5,20+7.75,30+12.25))
```

 - risque relatif a priori : $\frac{\mathbb{P}(\theta \leq 0.5)}{\mathbb{P}(\theta > 0.5)} = \frac{\int \mathbb{1}_{t\leq 0.5} \pi(\theta)d\theta}{\int \mathbb{1}_{t > 0.5} \pi(\theta)d\theta}$
```{r}
pbeta(0.5,7.75,12.25)/(1-pbeta(0.5,7.75,12.25))
```

 - facteur de Bayes : $BF = \frac{\mathbb{P}(\theta \leq 0.5 | x)}{\mathbb{P}(\theta > 0.5 | x)}  / \frac{\mathbb{P}(\theta \leq 0.5)}{\mathbb{P}(\theta > 0.5)}$
```{r}
(pbeta(0.5,20+7.75,30+12.25)/(1-pbeta(0.5,20+7.75,30+12.25))) / (pbeta(0.5,7.75,12.25)/(1-pbeta(0.5,7.75,12.25)))
```
...
```{r , echo=FALSE, fig.width=8, fig.height=6, fig.align='center', fig.retina=2}
a<-7.75;b<-12.25;nobs=50;xobs=20
dpost <- data.frame(x=seq(0,1,0.001),y=dbeta(x,20+7.75,30+12.25))
dprior <- data.frame(x=seq(0,1,0.001),y=dbeta(x,7.75,12.25))
ggplot(data=dpost,aes(x=x,y=y)) + stat_function(fun = dbeta, args=list(shape1=xobs+a,nobs-xobs+b),xlim=c(0,0.5),geom="area",fill="#77bc65",alpha=0.35) + xlim(c(0,1)) + geom_line(aes(col="Loi a posteriori"),lwd=1) + geom_line(data=dprior,aes(col="Loi a priori"),lwd=1) + stat_function(fun = dbeta, args=list(shape1=a,b),xlim=c(0,0.5),geom="area",fill="#ffd700",alpha=0.35) + scale_color_manual("",values=c("#77bc65","#ffd700")) + theme(legend.position = "bottom")
```
...
Echelle d'aide à la décision (Kass et Raftery, 1995)

</br>

```{r, echo=FALSE}
bf_1 <- data.frame(BF=c("1 < BF < 3","3 < BF < 20","20 < BF < 150","BF > 150"),
                   lnBF=c("entre 0 et 2","entre 2 et 6","entre 6 et 10","supérieur à 10"),
                   Force=c("Mérite à peine d'être mentionnée","Positive","Forte","Très forte"))
knitr::kable(bf_1, row.names = NA, align='cccc',
      col.names=c("BF","2 log(BF)","Force de la preuve pour H0"))
```


<span style="color:#16A085">**fin du cours 16**</span>
...
Fonction $\ell : \Theta \times \Theta \rightarrow \mathbb{R}^+$ telle que $\ell(\theta,\theta') = 0 \Leftrightarrow \theta = \theta'$.

.left-code[
```{r, echo=FALSE, fig.width=5, fig.height=6, fig.align='center', fig.retina=2, warning=FALSE, message=FALSE}
ecart <- seq(-2,2,0.01)
ql <- ecart^2
al <- abs(ecart)
hl <- ifelse(abs(ecart)<1.5,0.5*ql,1.5*al-0.5*1.5^2)
par(mar=c(8, 4, 4, 4), xpd=TRUE)
plot(ecart,ql,type="l",col="#0000CC",lwd=2,xlab=expression(theta-theta*"'"),ylab="perte")
lines(ecart,al,col="#CC0000",lwd=2)
lines(ecart,hl,col="#CC7700",lwd=2)
legend("bottom",legend=c("quadratique","absolue","Hubert"),lty=1,col=c("#0000CC","#CC0000","#CC7700"),bty="n",lwd=2,inset=c(0,-0.4),horiz=TRUE)
```
]
.right-plot[
- perte quadratique $\ell(\theta,\theta')=(\theta - \theta')^2$ : très utilisée en pratique
 - .green[.bold[avantages :]] convexe, différentiable
 - .red[.bold[inconvénients :]] donne un poids important aux erreurs extrêmes
- perte absolue $\ell(\theta,\theta')=|\theta - \theta'|$
 - .green[.bold[avantages :]] convexe, moins de poids sur les extrêmes
 - .red[.bold[inconvénients :]] non différentiable
- perte de Hubert : compromis entre les deux
$\small \ell(\theta,\theta')= \frac{1}{2} (\theta - \theta')^2 \mathbf{1}_{\theta-\theta'<k} + (k |\theta - \theta'| - \frac{1}{2}k^2)\mathbf{1}_{\theta - \theta' \geq k}$
]

---
# Fonction de risque

Fonction $R : \Theta \rightarrow \mathbb{R}^+$ telle que $R(\theta,T(\mathcal{X})) = \mathbb{E}_\theta (\ell(\theta,T(\mathcal{X}))) = \int_E \ell(\theta, T(x)) dP_\theta(x).$

**Exemple :** $\small X \sim \mathcal{B}(n,\theta)$, estimateurs :
- $\hat{\theta}_1 = \frac{X}{n}$ (EMV)
- $\hat{\theta}_2 = \frac{X + \sqrt{n}/2}{n+\sqrt{n}}$
- $\hat{\theta}_3 = \frac{1}{2}$ (estimateur constant)
- $\hat{\theta}_4 = \frac{X+1}{n+2}$ (moyenne a posteriori avec loi a priori uniforme sur $[0,1]$)

et perte quadratique.

--


```{r, echo=FALSE, fig.width=12, fig.height=3, fig.align='center', fig.retina=2, warning=FALSE, message=FALSE}
x <- seq(0,1,0.01)
n <- c(10,20,100,1000)
res <- lapply(n,FUN=function(ni){
r1 <- x*(1-x)/ni
r2 <- rep(1/4/(1+sqrt(ni))^2,length(x))
r3 <- (x-0.5)^2
a <- b <- 1
r4 <- (((a+b)^2-ni)*x^2 + (ni-2*a*(a+b))*x+a^2)/(ni+a+b)^2
d <- data.frame(x=rep(x,4),r=c(r1,r2,r3,r4),risque=rep(c("R1","R2","R3","R4"),each=length(x)),n=paste0("n=",rep(ni,4*length(x))))
return(d)
})
d <- do.call(rbind,res)
d$n <- factor(d$n,levels=c("n=10","n=20","n=100","n=1000"))
ggplot(data=d,aes(x=x,y=r,col=risque,group=risque)) + geom_line(lwd=1) + facet_wrap(~n,nr=1) + ylim(c(0,0.03)) + scale_color_brewer(palette="Dark2") +xlab(expression(theta))
```

---
# Fonction de risque

Zoom sur les 3 risques R1, R2 et R4.

```{r, echo=FALSE, fig.width=12, fig.height=3, fig.align='center', fig.retina=2, warning=FALSE, message=FALSE}
library(RColorBrewer)
coul <- brewer.pal(4,"Dark2")[c(1,2,4)]
d %>% filter(risque != "R3") %>% ggplot(aes(x=x,y=r,col=risque,group=risque)) + geom_line(lwd=1) + facet_wrap(~n,nr=1,scales="free") + scale_color_manual(values=coul) + xlab(expression(theta))
```

Chaque fonction de risque en fonction de $n$.

```{r, echo=FALSE, fig.width=12, fig.height=3, fig.align='center', fig.retina=2, warning=FALSE, message=FALSE}
ggplot(data=d,aes(x=x,y=r,col=n,group=n)) + geom_line(lwd=1) + facet_wrap(~risque,nr=1,scales="free") + xlab(expression(theta))#+ ylim(c(0,0.03))
```
...
Pour une loi a priori $\pi$, $R_{B} (\pi,T(\mathcal{X})) = \int_\Theta R(\theta,T(\mathcal{X})) \pi(\theta) d\theta$.

Suite exemple : modèle binomial, loi a priori $\small \mathcal{U}([0,1])$, $\hat{\theta}_1 = \frac{X}{n}$, $\hat{\theta}_2 = \frac{X + \sqrt{n}/2}{n+\sqrt{n}}$, $\hat{\theta}_3 = \frac{1}{2}$, $\hat{\theta}_4 = \frac{X+1}{n+2}$.


```{r, echo=FALSE, fig.width=8, fig.height=5, fig.align='center', fig.retina=2, warning=FALSE, message=FALSE}
n <- seq(10,100)
r1 <- 1/6/n
r2 <- 1/4/(1+sqrt(n))^2
r3 <- rep(1/12,length(n))
r4 <- 1/6/(n+2)
drb <- data.frame(n=rep(n,4),r=c(r1,r2,r3,r4),risque=rep(c("R1","R2","R3","R4"),each=length(n)))
ggplot(data=drb,aes(x=n,y=log(r),col=risque)) + geom_line(lwd=1) + scale_color_brewer(palette="Dark2")
```
...
magick::image_read_pdf("/home/charlotte/Documents/Enseignement/Lille/M1 ISN /Statistiques computationnelles/Poly/risque.pdf")
```
...
> **Théorème :** Pour toute loi a priori $\pi$, on a  $R_B(\pi) \leq R_M$

--

&rarr; le risque de Bayes est toujours plus petit que le risque minimax

--

> **Théorème :** Un estimateur de Bayes unique à équivalence près, pour une loi a priori $\pi$, est admissible.

--

&rarr; un estimateur de Bayes, c'est-à-dire qui minimise l'espérance de la fonction de perte par rapport à la loi a posteriori, est admissible. .red[.bold[On dispose donc une procédure pour construire des estimateurs admissibles]]

--

> **Théorème :** Un estimateur admissible et de risque constant est un estimateur minimax.

--

&rarr; **si on peut construire un estimateur de Bayes de risque constant, alors il sera minimax**

--

> **Théorème :** Un estimateur de Bayes pour une loi a priori $\pi$ dont la fonction de risque est majorée par son risque de Bayes est minimax.


<span style="color:#16A085">**fin du cours 17**</span>
...
Plusieurs cas possibles (combinaisons possibles) :
 1. utiliser l'information dont on dispose (dires d'experts, litérature, ...)

 3. utiliser des familles de lois conjuguées

 2. utiliser des a priori "impropres"

 4. utiliser des a priori "non informatifs"


.red[Dans tous les cas, le choix de la loi a priori doit être .bold[justifié] et doit être fait .bold[en amont], i.e. .bold[avant] d'avoir vu et analysé les données.]

.blue[.bold[En pratique, on peut aussi comparer les résultats obtenus avec différents a priori.]]

---
# 1. Prise en compte de l'information subjective

- pas toujours facile d'en déduire une **forme paramétrique** précise

- pas toujours facile de **combiner** les différentes sources d'information

&rarr; la loi a priori permet de résumer l'information disponible, mais aussi de prendre en compte l'incertitude entourant cette information

- biais cognitifs et difficulté à évaluer de petits (resp. grands) ordres de grandeur

---
# Quelques exemples de biais cognitifs

 - **biais de "proximité"** : on a tendance à accorder une plus grande probabilité à un évènement qui nous concerne ou que nous avons rencontré récemment

 - **biais d'ajustement et d'ancrage** : influence de la première impression ou information

 - **biais de confiance** : on a tendance à sous-estimer l'intervalle de variation d'une quantité (sous-évaluation de l'incertitude)

 - **biais d'équiprobabilité** : on à tendance à attribuer trop souvent la probabilité uniforme à un ensemble d'évènements. Lié aussi au biais existant lorsqu'on oublie la fréquence de base d'un évènement avant d'en évaluer des probabilités liées à cet évènement.

 - **biais d'autorité** : attribuer trop de valeur au jugement d'une personne dépositaire d'une autorité

 - **biais monétaire** : attribuer une plus grande efficacité aux processus/traitements plus chers

 - **biais de genre**, biais socio-culturels
...
Quelques approches :
 - identifier des quantités clés (moyenne, médiane, quantiles, ...) et en déduire une loi paramétrique

  - c'est ce que l'on a fait dans l'exemple Beta-binomial
  - dans ce cas il vaut mieux choisir une loi "plus large" que ce que suggèrent l'information

 - évaluer la robustesse des résultats au choix de la loi a priori

 - introduire un niveau hiérarchique supplémentaire dans la définition de la loi a priori (*modélisation Bayésienne hiérarchique*)

---
class: my-one-page-font
# 2. Utiliser des lois conjuguées

L'approche précédente n'est pas toujours possible :
 - Pas toujours d'information disponible ou de qualité

 - Volonté de rester le plus "objectif" possible, en utilisant juste l'information du modèle

--

Choisir un a priori conjugué :
 - permet d'utiliser un a priori qui ne dépend que du choix du modèle (i.e. de la vraisemblance)

 - limite la part de "subjectivité" au choix des paramètres de la loi a priori (et pas à sa forme paramétrique)

 - facilite l'analyse car la loi a posteriori est explicite

 - constitue tout de même un choix ! et à ce titre a une influence sur les résultats

 - peut être incompatible avec l'information dont on dispose (dans ce cas il ne faut pas l'utiliser !)


---
# Exemples de lois conjuguées

  Pour un échantillon $X_1,\dots,X_n$ iid

- Vraisemblance : $\mathcal{N}(\theta,\sigma^2)$, $\sigma$ connue &rarr; loi conjuguée Gaussienne.

--

- Vraisemblance $\mathcal{N}(\mu,1/\theta)$, $\mu$ connue &rarr; loi conjuguée Gamma.

--

- Vraisemblance $\mathcal{G}(k,\theta)$, $k$ connu &rarr; loi conjuguée Gamma.

--

- Vraisemblance $\mathcal{B}(\theta)$ &rarr; loi conjuguée Beta.

--

- Vraisemblance $\mathcal{P}(\theta)$ &rarr; loi conjuguée Gamma.

--

- Vraisemblance $\mathcal{NB}(m,\theta)$ &rarr; loi conjuguée Beta.

---
# Lois conjuguées et famille exponentielle

Une densité $f$ appartient à la **famille exponentielle** si elle s'écrit sous la forme :

$$f_\theta(x) = h(x) \exp \left( \langle \theta , T(x) \rangle - A(\theta) \right)$$

Si la loi des observations appartient à cette famille exponentielle, un choix  naturel de loi a priori conjuguée est :

$$\pi(\theta) = K(\mu,\lambda) \exp\left(\langle \theta , \mu \rangle - \lambda A(\theta)\right) $$

ce qui conduit à la loi a posteriori :

$$p(\theta | x) \propto \exp \left(\langle \theta, \mu + \sum_{i=1}^nT(x_i) \rangle - (\lambda + n) A(\theta) \right) $$
--

</br>
**Propriété :** si $T(X)=X$, la moyenne a posteriori est une combinaison linéaire de la moyenne a priori et de la moyenne empirique des observations :
$$\mathbb{E}(\theta | x) = \frac{\mu + n\bar{X}}{\lambda + n}$$

---
# 3. Loi a priori impropre

- Mesure sur $\Theta$ qui n'est pas nécessairement une mesure de probabilité (ni même une mesure de masse totale finie)

- Utilisable **uniquement** si la loi a posteriori qui en découle est une mesure de probabilité.

- **Exemple** : $X \sim \mathcal{N}(\theta,1)$ et $\pi(\theta) = 1$.

--

- .red[**La loi a posteriori peut ne pas être définie pour certaines observations**] &rarr; exemple dans le modèle Beta-binomial si prior impropre $\text{Beta}(0,0)$.

---
# 4. Lois a priori non-informatives

- A priori de Laplace &rarr; lois uniformes sur $\Theta$

 - peut conduire à des a priori impropres
 - pas d'invariance par reparamétrisation

--

- A priori de Jeffreys &rarr; lien avec l'information de Fisher

 - si $\theta \in \mathbb{R}$ : $\pi(\theta) \propto I^{1/2}(\theta)$
 - si $\theta \in \mathbb{R}^d, d> 1$ : $\pi(\theta) \propto \det(I(\theta))^{1/2}$

 - **invariant par reparamétrisation**

--

> **Proposition** : Soit $\pi$ la mesure a priori de Jeffreys pour $\theta$, et soit $\eta = g(\theta)$, avec $g$ un $\mathcal{C}^1$-difféomorphisme. Alors la mesure image de $\pi$ par $g$ est la mesure a priori de Jeffreys pour $\eta$.


<span style="color:#16A085">**fin du cours 18**</span>
...
# Identification de la loi a posteriori

---
# Loi a posteriori

**Cas 1 : expression exacte disponible** (famille de lois conjuguées, calcul explicite possible)

A partir de cette expression, on peut chercher à calculer :
 - moyenne, médiane, maximum a posteriori
 - quantiles pour régions de crédibilité
 - HDI
 - facteurs de Bayes

&rarr; pas toujours facile à calculer même à partir de l'expression analytique

--

**Cas 2 : expression connue à une constante près**

- Impossible de calculer moyenne ou médiane a posteriori ni intervalles de crédibilité, facteurs de Bayes, ...

- Toujours possible de calculer le mode a posteriori

--

&rarr; **on recourt à des approximations de type Monte-Carlo**

---
name: simu_post
# Simulations selon la loi a posteriori

Quelles méthodes utiliser ? [petit retour en arrière ...](#simu_f)

---
# Simulations selon la loi a posteriori

**Exemple** : exercice 3 de la fiche de TD 4, pour $\small X | \theta \sim \mathcal{B}(n,\theta)$ et $\small \theta \sim \mathcal{U}([1/2,1])$.
On a :

--

$\small p(\theta | x) = \frac{1}{1-F_{B(x+1,n-x+1)}} \frac{\Gamma(n+2)}{\Gamma(x+1)\Gamma(n-x+1)}\theta^x(1-\theta)^{n-x} \mathbb{1}_{[1/2,1]}(\theta)$
...
```{r, eval=FALSE}
n <- 10; X <- 3; c <- 1-pbeta(1/2,X+1,n-X+1)
p3 <- function(x){ifelse(x<1/2|x>1, 0, dbeta(x,X+1,n-X+1)/c)}
U <- runif(10000) ; ech_finv <- qbeta(c*U+pbeta(1/2,X+1,n-X+1), X+1, n-X+1)
```

```{r, echo=FALSE, fig.width=6, fig.height=4, fig.align='center', fig.retina=2, warning=FALSE, message=FALSE}
n <- 10; X <- 3; c <- 1-pbeta(1/2,X+1,n-X+1)
p3 <- function(x){ifelse(x<1/2|x>1,0,dbeta(x,X+1,n-X+1)/c)}

ech_finv <- qbeta(c*runif(10000)+pbeta(1/2,X+1,n-X+1),X+1,n-X+1)
x <- seq(0,1,0.001)
ggplot(data=NULL,aes(x=ech_finv)) + geom_histogram(aes(y=..density..),col="darkgrey",fill="grey",bins=30) + geom_line(aes(x=x,y=p3(x)),col="red")
```
...
**Exemple** : $\small p(\theta | x) = \frac{1}{1-F_{B(x+1,n-x+1)}} \frac{\Gamma(n+2)}{\Gamma(x+1)\Gamma(n-x+1)}\theta^x(1-\theta)^{n-x} \mathbb{1}_{[1/2,1]}(\theta)$
...
- acceptation-rejet &rarr;  $\small p(\theta|x) \leq M (2 \cdot \mathbb{1}_{[1/2,1]}(\theta))$

--

```{r, eval=FALSE}
n <- 10; X <- 3; c <- 1-pbeta(1/2,X+1,n-X+1)
M <- dbeta(1/2,X+1,n-X+1)/c
...
```{r, echo=FALSE, fig.width=8, fig.height=4, fig.align='center', fig.retina=2, warning=FALSE, message=FALSE}
n <- 10; X <- 3; c <- 1-pbeta(1/2,X+1,n-X+1)
M <- dbeta(1/2,X+1,n-X+1)/c
pp1 <- ggplot(data=NULL,aes(x=x,y=p3(x))) + geom_line(col="red") + geom_line(aes(x=x,y=ifelse(x<1/2,0,M)))

Y <- runif(10000,1/2,1)
U <- runif(10000,0,M)
ech_ar <- Y[U<p3(Y)]
pp2 <- ggplot(data=NULL,aes(x=ech_ar)) + geom_histogram(aes(y=..density..),col="darkgrey",fill="grey",bins=30) + geom_line(aes(x=x,y=p3(x)),col="red")

gridExtra::marrangeGrob(list(pp1,pp2),ncol=2,nrow=1,top="")
```

---
# Simulations selon la loi a posteriori

**Exemple** : $\small p(\theta | x) \propto \tilde{p}(\theta | x)$ avec $\tilde{p}(\theta|x) =  \theta^x(1-\theta)^{n-x} \mathbb{1}_{[1/2,1]}(\theta)$

- acceptation-rejet : fonctionne aussi si on connaît la loi a posteriori à une constante près
...
- acceptation-rejet : fonctionne aussi si on connaît la loi a posteriori à une constante près &rarr;  $\small \tilde{p}(\theta|x) \leq M (2 \cdot \mathbb{1}_{[1/2,1]}(\theta))$ avec $M=1/2^{n+1}$.

--

```{r, eval=FALSE}
Mtilde <- 1/2^(n+1)
ptilde3 <- function(x){ifelse(x<1/2|x>1,0,x^(X)*(1-x)^(n-X))}
Y <- runif(10000,1/2,1)
U <- runif(10000,0,1)
ech_ar_tilde <- Y[U<(ptilde3(Y)/Mtilde/dunif(Y,1/2,1))]
```

```{r, echo=FALSE, fig.width=4, fig.height=3, fig.align='center', fig.retina=2, warning=FALSE, message=FALSE}
n <- 10; X <- 3;
Mtilde <- 1/2^{n+1}
ptilde3 <- function(x){ifelse(x<1/2|x>1,0,x^(X)*(1-x)^(n-X))}
...
U <- runif(10000,0,1)
ech_ar_tilde <- Y[U<(ptilde3(Y)/Mtilde/dunif(Y,1/2,1))]
ggplot(data=NULL,aes(x=ech_ar_tilde)) + geom_histogram(aes(y=..density..),col="darkgrey",fill="grey",bins=30) + geom_line(aes(x=x,y=p3(x)),col="red")
```
...
```{r, eval=FALSE}
Z1 <- runif(20000,1/2,1)
w <- p3(Z1)/dunif(Z1,1/2,1)
ech_is <- sample(Z1,prob = w/sum(w),10000)
Z2 <- runif(100000,1/2,1)
w <- p3(Z2)/dunif(Z2,1/2,1)
ech_is_2 <- sample(Z2,prob = w/sum(w),10000)
```
...
pp1 <- ggplot(data=NULL,aes(x=ech_is)) + geom_histogram(aes(y=..density..),col="darkgrey",fill="grey",bins=30) + geom_line(aes(x=x,y=p3(x)),col="red") + ggtitle("Taille échantillonneur préférentiel : 20000")

pp2 <- ggplot(data=NULL,aes(x=ech_is_2)) + geom_histogram(aes(y=..density..),col="darkgrey",fill="grey",bins=30) + geom_line(aes(x=x,y=p3(x)),col="red") + ggtitle("Taille échantillonneur préférentiel : 100000")
...
- Metropolis-Hastings à loi instrumentale indépendante

--

```{r}
chaine1 <- rep(0,10000); chaine1[1] <- runif(1,1/2,1)
chaine2 <- rep(0,10000); chaine2[1] <- runif(1,1/2,1)
for (i in 2:10000){
# loi instrumentale U([1/2,1])
cand1 <- runif(1,1/2,1)
alpha1 <- min(1,p3(cand1)/p3(chaine1[i-1])*dunif(chaine1[i-1])/dunif(cand1))
chaine1[i] <- ifelse(runif(1)<alpha1,cand1,chaine1[i-1])

# loi instrumentale Beta(X+1,n-X+1)
cand2 <- rbeta(1,X+1,n-X+1)
alpha2 <- min(1,p3(cand2)/p3(chaine2[i-1])*dbeta(chaine2[i-1],X+1,n-X+1)/dbeta(cand2,X+1,n-X+1))
chaine2[i] <- ifelse(runif(1)<alpha2,cand2,chaine2[i-1])
}
```

```{r, echo=FALSE, results='hold'}
print(paste("Taux d'acceptation avec loi instrumentale U([1/2,1]) :",length(unique(chaine1))/length(chaine1)))
print(paste("Taux d'acceptation avec loi instrumentale Beta(X+1,n-X+1) :",length(unique(chaine2))/length(chaine2)))
```
...
```{r, echo=FALSE, fig.width=12, fig.height=6, fig.align='center', fig.retina=2, warning=FALSE, message=FALSE}
bacf <- acf(chaine1, plot = FALSE)
bacfdf1 <- with(bacf, data.frame(lag, acf))
bacf <- acf(chaine2, plot = FALSE)
bacfdf2 <- with(bacf, data.frame(lag, acf))

q1 <- ggplot(data = bacfdf1, mapping = aes(x = lag, y = acf)) +
geom_hline(aes(yintercept = 0)) +
geom_segment(mapping = aes(xend = lag, yend = 0)) + ggtitle("Loi instrumentale U([1/2,1])")
q2 <- ggplot(data = bacfdf2, mapping = aes(x = lag, y = acf)) +
geom_hline(aes(yintercept = 0)) +
geom_segment(mapping = aes(xend = lag, yend = 0)) + ggtitle("Loi instrumentale Beta(X+1,n-X+1)")

trace1 <- ggplot(data=NULL,aes(x=1:10000,y=chaine1)) + geom_line() + xlab("Itération")
trace2 <- ggplot(data=NULL,aes(x=1:10000,y=chaine2)) + geom_line() + xlab("Itération")

h1 <- ggplot(data=NULL,aes(x=chaine1)) +  geom_histogram(aes(y=..density..),col="darkgrey",fill="grey",bins=30) + geom_line(aes(x=x,y=p3(x)),col="red")

h2 <- ggplot(data=NULL,aes(x=chaine2)) +  geom_histogram(aes(y=..density..),col="darkgrey",fill="grey",bins=30) + geom_line(aes(x=x,y=p3(x)),col="red")

gridExtra::marrangeGrob(list(q1,q2,trace1,trace2,h1,h2),ncol=3,nrow=2,top="")
```
...
```{r}
chaine <- rep(0,10000); chaine[1] <- runif(1,1/2,1)
for (i in 2:10000){
cand <- chaine[i-1] + rnorm(1,0,0.1)
alpha <- min(1,p3(cand)/p3(chaine[i-1]))
chaine[i] <- ifelse(runif(1)<alpha,cand,chaine[i-1])}
```

```{r, echo=FALSE}
print(paste("Taux d'acceptation :",length(unique(chaine))/length(chaine)))
```
```{r, echo=FALSE, fig.width=10, fig.height=3.5, fig.align='center', fig.retina=2, warning=FALSE, message=FALSE}
bacf <- acf(chaine, plot = FALSE)
bacfdf1 <- with(bacf, data.frame(lag, acf))
...
trace1 <- ggplot(data=NULL,aes(x=1:10000,y=chaine)) + geom_line() + xlab("Itération")

h1 <- ggplot(data=NULL,aes(x=chaine)) +  geom_histogram(aes(y=..density..),col="darkgrey",fill="grey",bins=30) + geom_line(aes(x=x,y=p3(x)),col="red")

gridExtra::marrangeGrob(list(q1,trace1,h1),ncol=3,nrow=1,top="")
```
...
Que faire de ces échantillons (i.i.d. ou issus de chaînes de Markov) ?
- calcul de la moyenne, médiane, maximum a posteriori
- calcul d'intervalles de crédibilité, de facteurs de Bayes

**Exemple (suite) :** estimation de la moyenne a posteriori (vraie valeur : `r round((X+1)/(n+2)*(1-pbeta(1/2,X+2,n-X+1))/(1-pbeta(1/2,X+1,n-X+1)),4)`).

```{r, echo=FALSE, message=F}
library(kableExtra)
library(coda)
tab <- data.frame(method=c("Fonction inverse","Acceptation-rejet (AR)","AR sans la constante",
                           "EP de taille 20000","EP de taille 100000","MH à proposition indépendante",
                           "MH à marche aléatoire U([1/2,1])","MH à marche aléatoire Beta(X+1,n-X+1)"),
                  m=c(mean(ech_finv),mean(ech_ar),mean(ech_ar_tilde),mean(ech_is),mean(ech_is_2),mean(chaine1),mean(chaine2),mean(chaine)),
                  v=c(sd(ech_finv)/sqrt(10000),sd(ech_ar)/sqrt(length(ech_ar)),sd(ech_ar_tilde)/sqrt(length(ech_ar_tilde)),sd(ech_is)/sqrt(10000),sd(ech_is_2)/sqrt(10000),summary(mcmc(chaine1))$statistics[4],summary(mcmc(chaine2))$statistics[4],summary(mcmc(chaine))$statistics[4]),
                  n=c(10000,length(ech_ar),length(ech_ar_tilde),10000,10000,round(effectiveSize(mcmc(chaine1))),round(effectiveSize(mcmc(chaine2))),round(effectiveSize(mcmc(chaine)))))

tab[4,4] <- cell_spec(tab[4,4], color="red")
tab[5,4] <- cell_spec(tab[5,4], color="red")
knitr::kable(tab,row.names = NA, align='lccc',digits=c(0,4,6,0),
             col.names=c("Méthode","Moyenne a posteriori","Ecart-type","Taille (eff) éch"))
```

---
# Simulations selon la loi a posteriori

Comparaison des résultats de la diapo précédente :
 - la **meilleure méthode** au sens de l'écart-type de l'estimateur est celle de la **fonction inverse**
 - pour cette approche, on a généré 10000 simulations iid de loi uniforme pour un obtenir échantillon iid de taille 10000 selon la loi cible
 - pour l'acceptation-rejet, on a obtenu un échantillon iid de taille env. 1750 selon la loi cible, soit env. 10 fois plus petit que pour la fonction inverse &rarr; l'écart-type est donc env. $\sqrt{10}$ fois plus grand
 - les **deux échantillons obtenus par EP** ont en fait été générés à partir d'échantillons de taille beaucoup plus grande &rarr; ils peuvent atteindre une variance similaire à la méthode de la fonction inverse mais sont bien **plus coûteux**
 - les **méthodes MCMC**, à cause de la corrélation entre les simulations, conduisent à des **écart-types plus élevés** que dans le cas iid


---
# Simulations selon la loi a posteriori

Pour calculer l'écart-type associé aux méthodes MCMC :
 - soit on "élague" la chaine en ne gardant qu'une valeur toutes les $m$ itérations, où $m$ est choisi de telle sorte que l'autocorrélation entre $X_n$ et $X_{n-m}$ soit négligeable, et on calcule l'écart-type sur le vecteur obtenu (qui sera donc de taille plus petite)
 - soit on utilise le package `ccoda` et la fonction `summary()` qui renvoie l'écart-type estimé par des méthodes de séries temporelles

```{r}
# méthode 1
chaine_elaguee <- chaine[seq(1,length(chaine),by=20)] # d'après le graphe d'autocorrélation
sd(chaine_elaguee)/sqrt(length(chaine_elaguee))
