...
.red[**Conditions "nécessaires" mais pas suffisantes**]
&rarr; permet de détecter un problème, mais ne garantit pas que tout s'est bien passé !
 - choix d'une période de *burn-in*
 - convergence vers la loi stationnaire (pls initialisations)
```{r, echo=FALSE}
...
.pull-left[
```{r, echo=FALSE, fig.align='center',fig.height=3.25,fig.width=4.55,fig.retina=2}
...
.pull-right[
```{r, echo=FALSE, fig.align='center',fig.height=3.25,fig.width=4.5,fig.retina=2, message=FALSE, warning=FALSE}
...
```
 ...
 - considérer des sous-échantillons pour réduire la dépendance
 - package `coda` dans `R` (tests de stationnarité)
...
**Méthode :** <span style="color:red"> **obtenir des réalisations $X_1,\dots,X_n$ selon la loi $f$**</span> et poser $\hat{\mu} = \frac{1}{n}\sum_{i=1}^n h(X_i)$

--

1. Obtenir des réalisations selon la loi $f$
...
<ol start="2">
<li> Poser $\hat{\mu} = \frac{1}{n}\sum_{i=1}^n h(X_i)$
</ol>
...
---
# Algorithmes MCMC - monitoring

1. Tracer l'évolution de la chaîne &rarr; ''hairy caterpillar''

<img src="hairycaterpillar.png">
...
2. Tracer l'autocorrélation de la chaîne
...
3. Calculer le taux d'acceptation (MH)
 - le maximiser dans algo MH à loi instrumentale indépendante
 - proche de 0.44 en dimn. 1 et de 0.25 en dim > 1 dans MH à MA
 - proche de 0.57 dans algo MALA


---
# Diagnostics de convergence

**1. Convergence vers la loi stationnaire**

- le trace plot (évolution de la chaîne au cours des itérations) pour détecter les fortes non-stationnarités
- tests de stationnarité (e.g. Kolmogorov-Smirnov ou test de Heidelberg et Welch)
 - fonction `ks.test()`
 - package `coda` et fonction `heidel.diag()`
- exploration du support de $f$ &rarr; si $f$ connue exactement et si $d$ petit
 - approximation par sommes de Riemann
 - estimation de la densité

**2. Convergence en moyenne**

- tracer l'évolution de la moyenne cumulée et chercher la stabilisation
- idem pour les quantiles
- variance inter- et intra (plusieurs chaînes) &rarr; fonction `gelman.diag()` du package `coda`
- effective sampling size &rarr; fonction `effectiveSamplingSize()` du package `coda`
